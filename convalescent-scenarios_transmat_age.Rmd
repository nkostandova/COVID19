...
-$p_{give}$ = proportion of recovered individuals that give plasma (default: 0.1 if in period of time when plasma is being collected)
...
We allow the following parameters to vary over time:  R0 and plasma parameters. We assume that no plasma is collected until Phase I reopening (p_{give}=0 during the first three phases, and p_{give}=0.1 of recovered individuals otherwise).
...
seir_step_age_timevar = function(stoch.init = FALSE, stoch = TRUE, Ncomp = Ncomp, ICs = InCs, sd.dw = 0.00, 
  ...
  for (it in 1:(length(time) - 1)) {
    ...
    for (k in 1:Ncomp){
      ...
      Tm_a[2,] = c(wane_St, max(0,1-t(foi_prob_St[k])), t(foi_prob_St[k]), 0, 0, 0, 0, 0, 0, 0, 0, 0)
